...
# Check if the post-options argument is provided and matches /^ssh /'
...
export HOST=$(echo "$SSH_COMMAND" | perl -ne 'print $1 if /@(\S+)/')
...
if [ -z "$HOST" -o -z "$PORT" -o -z "$USER" ]; then
    echo "Format of ssh command must be: ssh -p <port> <user>@<host>..."
    exit 1
fi
