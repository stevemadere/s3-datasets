...
long_texts:list[str] = [open(f"{fixture_dir}/{f}").read() for f in txt_fixture_files]
...
def test_yield_tokenized_chunks_from_text_item() -> None:
    ...
    a_long_text = long_texts[0]
    source_dataset = datasets.Dataset.from_dict({"text": [a_long_text]})

    chunk_len=4096
    min_stride = 64
    max_waste = 64
    generator:TextDS2TokensGenerator = TextDS2TokensGenerator(source_dataset,tokenizer, chunk_len=chunk_len, min_stride= min_stride, max_waste=max_waste)
    item:DSItem = cast(DSItem, source_dataset[0])
    ...
    assert beginning_text == a_long_text[0:len(beginning_text)]
    assert ending_text == a_long_text[-len(ending_text):]
    if len(chunks)>2:
        second_text = tokenizer.decode(chunks[1]["input_ids"])
        position_of_second_text_in_long_text = a_long_text.find(second_text)
        assert position_of_second_text_in_long_text > max_waste 
        position_of_ending_text = a_long_text.find(ending_text)
        assert position_of_second_text_in_long_text < position_of_ending_text

    #print(f"beginning_text:\n{beginning_text}\n\n============================")
    #print(f"second_text:\n{second_text}\n\n============================")
    #print(f"ending_text:\n{ending_text}\n\n===========================")

# TextDS2TokensGenerator
def test_text_ds_2_tokens_generator_one_doc() -> None:
    tokenizer.pad_token = tokenizer.eos_token
    a_long_text = long_texts[0]
    source_dataset = datasets.Dataset.from_dict({"text": [a_long_text]})
...
# TextDS2TokensGenerator
def test_text_ds_2_tokens_generator_multi_doc() -> None:
    tokenizer.pad_token = tokenizer.eos_token
    texts = long_texts[0:3]
    source_dataset = datasets.Dataset.from_dict({"text": texts })
